...
#ifndef infinity_GARBAGECOLLECTOR_H
#define infinity_GARBAGECOLLECTOR_H
...
struct mutex_t
{
    ...
    mutex_t(infinityObject *o, recursive_mutex *mut, long threadid)
    : 
        object(o),
        mutex(mut),
        threadid(threadid)
...
public:
    ...
    infinityObject* newObject(int64_t size, unsigned short ntype); /* Array allocation */
    infinityObject* newObjectUnsafe(int64_t size, unsigned short ntype); /* Unconventional Array allocation */
    infinityObject* newObject(ClassObject* k, bool staticInit = false); /* Class allocation */
    infinityObject* newObjectUnsafe(ClassObject* k, bool staticInit = false); /* Class allocation */

    infinityObject* newObjectArray(int64_t size); /* Array Object allocation */
    infinityObject* newObjectArray(int64_t size, ClassObject* k); /* Class Array allocation */
     ...
     * @param sz
     */
    void realloc(infinityObject *o, size_t sz);
    void reallocObject(infinityObject *o, size_t sz);
     ...
     * @return
     */
    static double _sizeof(infinityObject *object);

    /**
     * Lock an object to be thread safe
     *
     */
    void lock(infinityObject *, Thread*);
    void unlock(infinityObject *, Thread*);
...
private:
     ...
     * infinity objects in the heap
     * @param object
     * @return
     */
    infinityObject* sweep(infinityObject *object, infinityObject *prevObj);

    void sedateSelf();

    void dropLock(infinityObject *);
...
#endif //infinity_GARBAGECOLLECTOR_H
