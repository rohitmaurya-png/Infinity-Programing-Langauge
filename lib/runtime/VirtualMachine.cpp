...
void invokeDelegate(int64_t address, int32_t args, Thread* thread, bool isStatic) {
    ...
    if(isStatic) {
        ...
        infinityObject* obj = (thread->sp-args)->object.object;
...
void VirtualMachine::getStackTrace() {
    ...
    infinityObject *frameInfo = (thread->sp)->object.object;
...
void VirtualMachine::sysInterrupt(int64_t signal) {
    switch (signal) {
        ...
        case OP_PRINTF:
            ...
            return;
        case OP_STRTOD: {
            infinityObject *str = (thread_self->sp--)->object.object;
        ...
        case OP_RENAME_FILE:
        case OP_WRITE_FILE:  {
            infinityObject *pathObj = (thread_self->sp--)->object.object;
            infinityObject *newNameObj = (thread_self->sp--)->object.object;
        ...
        case OP_LOAD_LIBRARY: {
            infinityObject *libNameObj = (thread_self->sp--)->object.object;
        ...
        case OP_LINK_FUNC: {
            infinityObject *libNameObj = (thread_self->sp--)->object.object;
            infinityObject *funcNameObj = (thread_self->sp--)->object.object;
        ...
        if (pc >= tbl->block_start_pc && pc <= tbl->block_end_pc)
        {
            for(Int j = 0; j < tbl->catchTable.len; j++) {
                if(handlingClass->guid == tbl->catchTable._Data[j].caughtException->guid
                    ...
                    thread->exceptionObject = (infinityObject*)NULL;
...
void VirtualMachine::fillStackTrace(infinityObject *frameInfo, infinityObject *stackTrace) {
...
Object *VirtualMachine::resolveField(std::string name, infinityObject *classObject) {
...
bool VirtualMachine::isStaticObject(infinityObject *object) {
    return object != NULL && GENERATION(object->info) == gc_perm;
}

double VirtualMachine::numberValue(Int index, infinityObject *object) {
    if(object != NULL) {
...
string VirtualMachine::stringValue(infinityObject *object) {
    stringstream ss;
    if(object != NULL) {
        if (TYPE(object->info) == _stype_var) {
...
void VirtualMachine::setFieldVar(std::string name, infinityObject *classObject, Int index, double value) {
...
void VirtualMachine::setFieldClass(std::string name, infinityObject *classObject, ClassObject *klass) {
    Object *field = vm.resolveField(std::move(name), classObject);
    if(field != NULL) {
