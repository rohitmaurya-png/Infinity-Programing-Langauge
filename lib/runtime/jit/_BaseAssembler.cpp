//
//Created by Rohit Maurya 8/21/2019.
//
...
void _BaseAssembler::setupThreadFields() {
    ...
    Lthread[thread_stack_lmt] = getMemPtr(relative_offset(thread, calls, stackLimit));
    ...
    Lthread[thread_stack] = getMemPtr(relative_offset(thread, calls, stackSize));
}
...
void _BaseAssembler::setupinfinityObjectFields() {
    infinityObject o;
    Linfinity_object[infinity_object_HEAD] = getMemPtr(relative_offset((&o), HEAD, HEAD));
    Linfinity_object[infinity_object_node] = getMemPtr(relative_offset((&o), HEAD, node));
    Linfinity_object[infinity_object_size] = x86::dword_ptr(ctx, relative_offset((&o), HEAD, size));
    Linfinity_object[infinity_object_info] = x86::dword_ptr(ctx, relative_offset((&o), HEAD, info));
}
            ...
            for(x86int_t i = 0; i < func->cacheSize; i++) {
                ...
                switch(GET_OP(ir)) {
                    ...
                    case op_IALOAD: { // change
                        ...
                        assembler.mov(tmp32, Linfinity_object[infinity_object_info]);
                        ...
                        assembler.mov(value, Linfinity_object[infinity_object_HEAD]);
                    ...
                    case op_SIZEOF: {
                        ...
                        assembler.mov(ctx, Linfinity_object[infinity_object_size]);
                ...
                CHECK_NULL2(
                        ...
                        assembler.mov(fnPtr, Linfinity_object[infinity_object_size]);
...
void _BaseAssembler::checkO2Node(x86::Assembler &assembler, const x86::Mem &o2Ptr, const Label &thread_check, x86int_t pc) {
    ...
    assembler.mov(ctx, Linfinity_object[infinity_object_node]);
    assembler.jmp(nullCheckPassed);
...
void _BaseAssembler::checkO2Head(x86::Assembler &assembler, const x86::Mem &o2Ptr, const Label &thread_check, x86int_t pc) {
    ...
    assembler.mov(ctx, Linfinity_object[infinity_object_HEAD]);
    assembler.jmp(nullCheckPassed);
...
infinityObject* _BaseAssembler::jitNewObject(x86int_t size) {
    try {
...
infinityObject* _BaseAssembler::jitNewObject2(x86int_t size) {
    try {
...
infinityObject* _BaseAssembler::jitNewClass0(x86int_t classid) {
//    try {
...
infinityObject* _BaseAssembler::jitNewClass1(x86int_t size, x86int_t classid) {
//    try {
...
void _BaseAssembler::jitSetObject0(infinityObject* o, StackElement *sp) {
    sp->object = o;
}
...
void _BaseAssembler::jitSetObject3(Object *dest, infinityObject *src) {
    *dest = src;
}
