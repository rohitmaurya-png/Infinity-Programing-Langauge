...
/**
 * API level thread create called from infinity
 *
 * @param methodAddress
 * @param stack_size
 * @return
...
string getVarCastExceptionMsg(DataType varType, bool isArray, infinityObject *obj) {
...
void Thread::exec() {
    ...
    try {
        for (;;) {
            ...
            SWAP:
                if((sp-dataStack) >= 2) {
                    infinityObject *swappedObj = sp->object.object;
