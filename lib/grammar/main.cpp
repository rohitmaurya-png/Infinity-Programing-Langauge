...
void get_full_file_list(native_string &path, List<native_string> &files) {
    ...
    if ((dir = opendir (path.str().c_str())) != NULL) {
        ...
        while ((ent = readdir (dir)) != NULL) {
            ...
            if(ends_with(file.str(), ".infinity")) {
                files.__new().init();
                files.last() = file;
            }
        }
...
void help() {
    cout << "Usage: infinityc " << "{OPTIONS} SOURCE FILE(S)" << std::endl;
    cout << "Source file must have a .infinity extion to be compiled.\n" << endl;
    ...
    cout <<               "    -target           target the specified platform of infinity to run on"      << endl;
...
void error(string message) {
    cout << "infinityc:  error: " << message << endl;
    exit(1);
}
...
int _bootstrap(int argc, const char* argv[])
{
...
#ifdef WIN32_
    native_string path("C:/Program Files/infinity/include");
#endif
#ifdef POSIX_
    native_string path("/usr/include/infinity/");
#endif
    ...
    for(unsigned int i = 0; i < files.size(); i++) {
        ...
        if(!File::endswith(".infinity", file)){
            error("file `" + file + "` is not a infinity file!");
        }
    }
