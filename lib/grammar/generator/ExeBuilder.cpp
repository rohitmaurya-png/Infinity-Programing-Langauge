...
void ExeBuilder::createNativeHeaderFile() {
    stringstream ss;
    ss << "#ifndef infinity_NATIVE_MAPPING_H" << endl;
    ss << "#define infinity_NATIVE_MAPPING_H" << endl << endl;
    ...
    ss << "#endif //infinity_NATIVE_MAPPING_H" << endl;
...
void ExeBuilder::appendinfinityMappingSourceFile(ClassObject* klass, stringstream& ss) {
    List<Method*> &classMethods = klass->getFunctions();
...
void ExeBuilder::createinfinityMappingSourceFile() {
    stringstream ss;
    ss << "#include \"native_mapping.h\"" << endl
       << endl
    ...
    for(Int j = 0; j < allClasses.size(); j++) {
        appendinfinityMappingSourceFile(allClasses.get(j), ss);
    }

#ifdef _WIN32
    string mappingFile = c_options.nativeCodeDir + "\\infinity_mapping.cpp";
    _mkdir(c_options.nativeCodeDir.c_str());
#else
    string mappingFile = c_options.nativeCodeDir + "/infinity_mapping.cpp";
    mkdir(c_options.nativeCodeDir.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
...
void ExeBuilder::createNativeSourceCode() {
    ...
    createinfinityMappingSourceFile();
}
